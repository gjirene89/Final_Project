...
bool CBlock::InitializeObject(ID3D11Device* device, ID3D11DeviceContext* deviceContext)
{
	...
	result = m_model->Initialize(device, "../Final_Project/Resources/Model/block.txt");
	if (!result)
	{
		return false;
	}

	m_model->LoadColorMap(device, deviceContext, "../Final_Project/Resources/Texture/blockColor.tga");
	m_model->LoadBumpMap(device, deviceContext, "../Final_Project/Resources/Texture/blockBump.tga");
